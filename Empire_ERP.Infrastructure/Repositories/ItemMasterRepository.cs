...
namespace Empire_ERP.Infrastructure.Repositories
{
    public class ItemMasterRepository : IItemMasterRepository
    {
                        ...
                        try
                        {
                            ...
                            if (modelRecord.ITEM_CODE == null || modelRecord.ITEM_CODE == 0)
                            {
                                ...
                                query = "INSERT INTO " + table + " " +
                                        "(ITEM_CODE,ITEM_ID,ITEM_NAME,ITEM_SHORT_NAME," +
                                        ...
                                        "VALUES" +
                                        "('" + newItemCode + "','" + modelRecord.ITEM_ID + "','" + modelRecord.ITEM_NAME + "','" + modelRecord.ITEM_SHORT_NAME + "'," +
                ...
                if (!String.IsNullOrWhiteSpace(table) && !String.IsNullOrWhiteSpace(detailTable))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        string query = $@"SELECT A.ITEM_CODE,A.ITEM_ID,A.ITEM_NAME,A.ITEM_SHORT_NAME,A.REMARKS,A.GROUP_CODE,A.IUNIT_CODE,A.PACK,A.PUNIT_CODE,A.SALE_RATE,A.PURCHASE_RATE,
                                        ...
                                        S.GROUP_NAME AS SEASON, G.GROUP_NAME AS BRAND, ST.GROUP_NAME AS STYLE
                                        FROM {table} A 
                                        LEFT JOIN TBL_CATEGORY B ON A.CAT_CODE = B.GROUP_CODE 
                                        LEFT JOIN TBL_SUB_CATEGORY C ON A.SUB_CAT_CODE = C.GROUP_CODE 
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        string query = $@"SELECT IT.HS_CODE, IT.ITEM_CODE,IT.ITEM_ID,IT.ITEM_NAME,IT.ITEM_SHORT_NAME,IT.REMARKS,IT.GROUP_CODE,IT.IUNIT_CODE,IT.PACK,IT.PUNIT_CODE,IT.SALE_RATE,
                                        IT.PURCHASE_RATE,IT.SALESTAX,IT.ITAX_STATUS,
        ...
        public MyHttpResponseMessage GetItemSizesInfoByCode(int code, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                var Menu = _menuRepository.GetMenu(common.MenuID);
                string? table = "ItemSizes";
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string Qurey = $@"SELECT CODE, S.GROUP_NAME AS SIZE
                                            FROM TBL_ITEM_SIZE ITS
                                            LEFT OUTER JOIN TBL_SIZE S ON S.GROUP_CODE = ITS.SIZE_CODE
                                            WHERE ITS.ITEM_CODE = {code} AND ITS.DLT = 'T'";
        ...
        public MyHttpResponseMessage SaveItemSize(ItemAttribute modelRecord, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        try
                        {
                            ...
                            code = GenerateNextItemSizeId(common, command);

                            query = "INSERT INTO TBL_ITEM_SIZE " +
                                            "(CODE, ITEM_CODE, SIZE_CODE, ADD_USER_ID, ADD_DATE, ADD_COMPUTER_NAME, ADD_IP_ADDRESS," +
                                            "ADD_POSTALCODE, EDIT_USER_ID, EDIT_DATE, EDIT_COMPUTER_NAME, EDIT_IP_ADDRESS, EDIT_POSTALCODE, MENU_ID, DLT) " +
                                            "VALUES " +
                                            "('" + code + "','"
                                            + modelRecord.ITEM_CODE + "','"
                                            + modelRecord.ITEM_SIZE + "','"
                                            + userid + "','"
                                            + CommonService.GetDateTime("Pakistan Standard Time") + "','"
                                            + Computer + "','"
                                            + Ip + "','"
        ...
        private int GenerateNextItemSizeId(Common common, SqlCommand command)
        {
            try
            {
                var Menu = _menuRepository.GetMenu(common.MenuID);
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    string maxIdQuery = $"SELECT ISNULL(MAX(CODE), 0) + 1 FROM TBL_ITEM_SIZE ";
                    command.CommandText = maxIdQuery;
                    object result = command.ExecuteScalar();
                    return Convert.ToInt32(result);
                }
        ...
        public MyHttpResponseMessage DeleteItemSizeInfo(int sizeId, int sizeCode, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            response.msgType = 2;
            response.msg = "Data not found in our records";
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    else
                    {
                        ...
                        {
                            connection.Open();
                            query = "UPDATE " + table + " SET DLT = 'F' WHERE CODE = '" + sizeId + "' AND ITEM_CODE = '" + sizeCode + "'";
                            SqlCommand command = new SqlCommand(query, connection);
                            command.ExecuteNonQuery();
                            response.msgType = 1;
                            response.msg = "Size Deleted Successfully";
                        }
