    ...
    public class MerchantPurchaseOrderDetailRepository : IMerchantPurchaseOrderDetailRepository
    {
                            ...
                            try
                            {
                                ...
                                if (item.DT_CODE == null || item.DT_CODE == 0)
                                {
                                    ...
                                    if (detailCode > 0)
                                    {
                                        detailQuery = $@"INSERT INTO {detailTable}
                                                       (TRAN_ID, DT_CODE, ORDER_NO, COMM_TYPE, COMM_RATE, COMM_AMT, SIZE, COLOR, QTY, CARTON, TOTAL_CARTON, UNIT, PICK_ID,
                                                       ...
                                                       '{item.SIZE}','{item.COLOR}','{item.QTY}','{item.CARTON}','{item.TOTAL_CARTON}', '{item.UNIT}', '{modelRecord.Master.PICK_ID}',
                                ...
                                else if ((item.DT_CODE != null || item.DT_CODE != 0) && item.REV_TOGGLE != true)
                                {
                                    ...
                                    detailQuery = $"UPDATE {detailTable} " +
                                                  ...
                                                  $"QTY = '{item.QTY}', " +
                                                  $"CARTON = '{item.CARTON}', " +
                                                  $"TOTAL_CARTON = '{item.TOTAL_CARTON}', " +
                                                  $"UNIT = '{item.UNIT}', " +
                                                  $"RATE = '{item.RATE}', " +
                                                  $"AMT = '{item.AMT}', " +
        ...
        public MyHttpResponseMessage GetDataForReport(MerchantPurchaseOrderDetailRDLCReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            ...
            try
            {
                ...
                topQuery = $@"SELECT MPO.CLIENT_PO, P.PARTY_NAME AS SUP_NAME, F.GROUP_NAME AS FABRIC, G.GROUP_NAME AS GSM, PC.PARTY_NAME AS CLIENT_NAME, MPO.V_DATE, T.GROUP_NAME AS TERMS, C.DESCR AS CURRENCY , MPO.QTY, M.ADD_USER_ID AS USER_NAME
                                FROM {table} M
                                ...
                                LEFT OUTER JOIN TBL_CURRENCY C ON MPO.CURR_CODE = C.CODE
                                LEFT JOIN TBL_FABRIC F ON F.GROUP_CODE = M.FABRIC
								LEFT JOIN TBL_GSM G ON G.GROUP_CODE = M.GSM
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        masterData.FABRIC = reader["FABRIC"] == DBNull.Value ? "" : Convert.ToString(reader["FABRIC"]);
                        masterData.GSM = reader["GSM"] == DBNull.Value ? "" : Convert.ToString(reader["GSM"]);
                    }
                    reader.Close();
                }
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        dataRow["DT_DESC"] = reader["DT_DESC"] == DBNull.Value ? string.Empty : reader["DT_DESC"].ToString();
