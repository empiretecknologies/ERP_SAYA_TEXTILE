                        ...
                        try
                        {
                            ...
                            if (modelRecord.TRAN_ID == 0 || modelRecord.TRAN_ID is null)
                            {
                                ...
                                query = "INSERT INTO " + table + " " +
                                        ...
                                        "[CLIENT_PO],[DEP_ID],[QTY],[RATE],[AMT],[CURR_CODE],[CRATE],[TERMS],[COMM_AMT],[COMM],[OTHER_INCOME],[NET_AMT],[COMM_VAL],[REMARKS],[ITEM_CODE],[GRADE],[DOC],[BCODE],[PERIOD_ID]," +
                                        ...
                                        "'" + modelRecord.QTY + "','" + modelRecord.RATE + "','" + modelRecord.AMT + "','" + modelRecord.CURR_CODE + "','" + modelRecord.CRATE + "','" + modelRecord.TERMS + "','" + modelRecord.COMM_AMT + "','" + modelRecord.COMM + "','" + modelRecord.OTHER_INCOME + "','" + modelRecord.NET_AMT + "','" + modelRecord.COMM_VAL + "'," +
                            ...
                            else
                            {
                                ...
                                string checkQuery = $@"SELECT PSD.ITEM_CODE
                                                        FROM TBL_MPOD_MASTER PSM
                                                        LEFT OUTER JOIN TBL_MPO_MASTER PO ON PO.TRAN_ID = PSM.JOB_NO
                                                        LEFT OUTER JOIN TBL_MPOD_DETAIL PSD ON PSD.TRAN_ID = PSM.TRAN_ID
                                                        WHERE PSM.JOB_NO = {modelRecord.TRAN_ID} AND PO.BCODE = PSM.BCODE AND PO.PERIOD_ID = PSM.PERIOD_ID AND PSM.DLT = 'T'
                                                        GROUP BY PSD.ITEM_CODE";

                                SqlCommand checkCommand = new SqlCommand(checkQuery, connection, transaction);
                                object result = checkCommand.ExecuteScalar();

                                if (result != null && result != DBNull.Value && Convert.ToInt32(result) != modelRecord.ITEM_CODE)
                                {
                                    response.msg = "You cannot change item , it is used in planing sheet !";
                                    response.msgType = 2;
                                    return response;
                                }


                            //    string checkPartyQuery = $@"SELECT TOP 1 PSM.SPARTY_CODE, PSM.SACT_CODE
                            //FROM TBL_MPOD_MASTER PSM
                            //LEFT OUTER JOIN TBL_MPO_MASTER PO ON PO.TRAN_ID = PSM.JOB_NO
                            //WHERE PSM.JOB_NO = {modelRecord.TRAN_ID}
                            //  AND PO.BCODE = PSM.BCODE 
                            //  AND PO.PERIOD_ID = PSM.PERIOD_ID 
                            //  AND PSM.DLT = 'T'";

                            //    using (SqlCommand checkPartyCommand = new SqlCommand(checkPartyQuery, connection, transaction))
                            //    {
                            //        using (SqlDataReader reader = checkPartyCommand.ExecuteReader())
                            //        {
                            //            if (reader.Read()) 
                            //            {
                            //                string spartyCode = reader["SPARTY_CODE"] != DBNull.Value ? Convert.ToString(reader["SPARTY_CODE"]) : "";
                            //                int sactCode = reader["SACT_CODE"] != DBNull.Value ? Convert.ToInt32(reader["SACT_CODE"]) : 0;

                            //                if (spartyCode != modelRecord.PARTY_CODE || sactCode != modelRecord.ACT_CODE)
                            //                {
                            //                    response.msg = "You cannot change party/account, it is used in planning sheet!";
                            //                    response.msgType = 2;
                            //                    return response;
                            //                }
                            //            }
                            ...
                            //    }
                                ...
                                query = "UPDATE " + table + " SET " +
                                        ...
                                        [OTHER_INCOME] = '" + modelRecord.OTHER_INCOME + @"',
                                        [NET_AMT] = '" + modelRecord.NET_AMT + @"',
        ...
        public MyHttpResponseMessage GetMerchantPurchaseOrderById(int id, Core.Entities.Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $@"SELECT * FROM {table} WHERE TRAN_ID = {id} AND DLT = 'T'";
                        ...
                        if (reader.Read())
                        {
                            var jsonDataResult = new
                            {
                                ...
                                OTHER_INCOME = reader["OTHER_INCOME"].ToString(),
                                NET_AMT = reader["NET_AMT"].ToString(),
        ...
        public MyHttpResponseMessage GetDataForReport(MerchantPurchaseOrderReport modelRecord, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            ...
            try
            {
                ...
                topQuery = $@"SELECT MPO.TRAN_ID, MPO.V_DATE, MPO.VOUCHER_NO, MPO.REF, MPO.CLIENT_PO, PTC.PARTY_NAME AS CLIENT_NAME, D.DESCR AS DEP,
                                    ...
                                    IT.ITEM_NAME,  G.GROUP_NAME AS BRAND, MPO.QTY, U.GROUP_NAME AS UNIT_NAME, MPO.RATE, MPO.AMT, MPO.OTHER_INCOME, MPO.NET_AMT,
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        masterData.OTHER_INCOME = reader["OTHER_INCOME"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["OTHER_INCOME"]);
                        masterData.NET_AMT = reader["NET_AMT"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["NET_AMT"]);
