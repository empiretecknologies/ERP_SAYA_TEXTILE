...
namespace Empire_ERP.Infrastructure.Repositories
{
    public class POSTransactionRepository : IPOSTransactionRepository
    {
                    ...
                    if (isStockSufficient)
                    {
                        using (SqlConnection connection = new SqlConnection(connectionString))
                        {
                            ...
                            try
                            {
                                ...
                                var itemDiscount = modelRecord.Detail.Sum(i => i.DISC_AMT);
                                        ...
                                        if (item.DT_CODE == null || item.DT_CODE == 0)
                                        {
                                            ...
                                            if (detailCode > 0)
                                            {
                                                ...
                                                detailQuery = $"INSERT INTO {detailTable} " +
                                                              "(TRAN_ID, DT_CODE, ITEM_CODE, QTY, UNIT, RATE, AMT, DISC, NET_AMT, RITEM, " +
        ...
        public MyHttpResponseMessage GetPOSTransactionDetailByCode(int? code, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        string query = $@"SELECT PD.DT_CODE, PD.ITEM_CODE, IT.ITEM_NAME , PD.QTY, PD.UNIT, UN.GROUP_NAME , 
        ...
        public MyHttpResponseMessage GetItemsMasterByCode(int itemId, string barcode, int Qty, int TranId, Common common)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    //{
                    //    query = $"SELECT IM.ITEM_CODE , IM.ITEM_ID , '' AS DISC, IM.ITEM_NAME , IM.SALE_RATE,IU.GROUP_NAME , IU.GROUP_CODE , '' As BARCODE , ISNULL(IG.SALES_TAX,0) AS TAX FROM TBL_ITEMSMASTER IM LEFT OUTER JOIN TBL_ITEMSGROUP IG ON IG.GROUP_CODE = IM.GROUP_CODE LEFT OUTER JOIN TBL_UNIT IU ON IU.GROUP_CODE = IM.IUNIT_CODE WHERE IM.DLT = 'T' And IM.ASTATUS = 'Y' And IM.ITEM_CODE = {itemId}";
                    //}
                    //else
                    //{
                    //    //query = $"SELECT IM.ITEM_CODE , IM.ITEM_ID , IM.ITEM_NAME , TB.RRATE As SALE_RATE ,IU.GROUP_NAME , IU.GROUP_CODE , TB.BARCODE , ISNULL(IG.SALES_TAX,0) AS TAX FROM TBL_ITEMSMASTER IM LEFT OUTER JOIN TBL_ITEMSGROUP IG ON IG.GROUP_CODE = IM.GROUP_CODE LEFT OUTER JOIN TBL_UNIT IU ON IU.GROUP_CODE = IM.IUNIT_CODE LEFT OUTER JOIN TBL_BARCODE TB ON TB.ITEM_CODE = IM.ITEM_CODE  WHERE IM.DLT = 'T' And IM.ASTATUS = 'Y' And TB.CODE = {itemId}";
                    //    query = $"SELECT IM.ITEM_CODE , IM.ITEM_ID , IM.ITEM_NAME , TB.RRATE As SALE_RATE ,IU.GROUP_NAME , IU.GROUP_CODE , TB.BARCODE , ISNULL(IG.SALES_TAX,0) AS TAX , IDM.DISC AS DISC FROM TBL_ITEMSMASTER IM " +
                    //        $"LEFT OUTER JOIN TBL_IT_DISC_DETAIL IDD ON IM.ITEM_CODE = IDD.ITEM_CODE\r\nLEFT OUTER JOIN TBL_IT_DISC_MASTER IDM ON IDM.CODE = IDD.CODE\r\nLEFT OUTER JOIN TBL_ITEMSGROUP IG ON IG.GROUP_CODE = IM.GROUP_CODE " +
                    //        $"LEFT OUTER JOIN TBL_UNIT IU ON IU.GROUP_CODE = IM.IUNIT_CODE \r\nLEFT OUTER JOIN TBL_BARCODE TB ON TB.ITEM_CODE = IM.ITEM_CODE  \r\nWHERE IM.DLT = 'T' And IM.ASTATUS = 'Y' And TB.CODE = {itemId}";
                    //}
                    ...
                    while (reader.Read())
                    {
                        var row = new
                        {
                            ...
                            STOCK = isStockSufficient ? "" : $"HAS ONLY {InSufficientItemQty} IN STOCK",
        ...
        //                CPC_CODE = currentBranch.CPC_CODE,
        ...
        //                FBLINK = currentMapping.FB_LINK,
        //                INSTALINK = currentMapping.INSTA_LINK,
        //                WEBLINK = currentMapping.WEB_LINK,
        //                TIKTOKLINK = currentMapping.TIKTOK_LINK,
        //                YOUTUBELINK = currentMapping.YOUTUBE_LINK,
        //                WIFIPASS = currentMapping.WIFI_PASSWORD,
        //                WIFINAME = currentMapping.WIFI_NAME,
        ...
        public int GetAvailableStock(int? barcodeId, string period, Common common)
        {
            ...
            string query = $@"EXEC STKPROC 26,'{StartDate}','{EndDate}','{common.Branch}','{common.Period}','','','','','{barcodeId}','','','B','','','',''";
            int stock = 0;
            using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
            {
                SqlCommand command = new SqlCommand(query, connection);
