    ...
    public class PartyReportRepository : IPartyReportRepository
    {
        ...
        public async Task<MyHttpResponseMessage> GetReportData(PartyReport report, Common common)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    string query = $"EXEC PPROC '{report.ReportID}','{report.FromDate.Value.ToString("yyyy-MM-dd")}','{report.ToDate.Value.ToString("yyyy-MM-dd")}','{report.ControlCode}','{report.PartyCode}','{common.Branch}','{common.Period}','{report.RegionCode}','{report.Nature}','{report.ISCURRENCY}'";
                    ...
                    else if (report.ReportID is 99)
                    {
                        while (reader.Read())
                        {
                            var row = new CustomPartyReport
                            {
                                ...
                                ChqNo = Convert.ToString(reader["CHQ_NO"]),
                                //ChqDate = reader["CHQ_DATE"] == DBNull.Value ? "" : Convert.ToDateTime(reader["CHQ_DATE"]).ToString("yyyy-MM-dd"),
                                ChqDate = reader["CHQ_DATE"] == DBNull.Value || Convert.ToDateTime(reader["CHQ_DATE"]).Year <= 1900 ? null : Convert.ToDateTime(reader["CHQ_DATE"]).ToString("yyyy-MM-dd"),
