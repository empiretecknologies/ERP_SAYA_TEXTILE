...
{
    public class PriceQuotedRepository : IPriceQuotedRepository
    {
        public IBranchRepository _branchRepository { get; set; }
        public IMenuService _menuRepository { get; set; }
        public PriceQuotedRepository(IBranchRepository branchRepository, IMenuService menuRepository)
        {
        ...
        public MyHttpResponseMessage Save(CustomPriceQuoted modelRecord, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                                ...
                                if (item.DT_CODE == null || item.DT_CODE == 0)
                                {
                                    ...
                                    if (detailCode > 0)
                                    {
                                        detailQuery = $@"INSERT INTO {detailTable}
                                                       (TRAN_ID, DT_CODE, SPARTY_CODE, SACT_CODE, YARN_CODE, COMM_RATE, PQ_DATE, RATE, PICK_ID, REV_STATUS, REV_REF,
                                                       ...
                                                       VALUES
                                                       ('{modelRecord.Master.TRAN_ID}', '{detailCode}', '{item.SPARTY_CODE}', '{item.SACT_CODE}', '{item.YARN_CODE}', '{item.COMM_RATE}', '{item.PQ_DATE}',
        ...
        public MyHttpResponseMessage GetPriceQuotedByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetPriceQuotedDetailByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    while (reader.Read())
                    {
                        var row = new
                        {
                            PQ_DATE = reader["PQ_DATE"] == DBNull.Value || Convert.ToDateTime(reader["PQ_DATE"]) == new DateTime(1900, 1, 1) ? null : Convert.ToDateTime(reader["PQ_DATE"]).ToString("yyyy-MM-dd"),
        ...
        public MyHttpResponseMessage GetBatchDetailByProcess(PriceQuoted model, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $@"WITH LatestRecords AS (
                                                ...
                                                SELECT 
                                                    ...
                                                    D.DT_CODE AS PICK_ID, D.SPARTY_CODE, D.SACT_CODE, SPT.PARTY_NAME AS SUPPLIER, D.YARN_CODE, Y.GROUP_NAME AS YARN, D.COMM_RATE, D.RATE
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                PriceQuoted PriceQuoted = new PriceQuoted();
                List<PriceQuotedDetail> PriceQuotedDetailList = new List<PriceQuotedDetail>();
                ...
                var customPriceQuoted = new CustomPriceQuoted
                {
                    Master = PriceQuoted,
                    Detail = PriceQuotedDetailList
                };

                response = this.Save(customPriceQuoted, common, menu);
        ...
        public MyHttpResponseMessage DeletePriceQuotedDetailByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetDataForReport(PriceQuotedRDLCReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            PriceQuotedRDLCReport masterData = new PriceQuotedRDLCReport();
            CustomPriceQuotedForPrintReport reportData = new CustomPriceQuotedForPrintReport();
            var Menu = _menuRepository.GetMenu(common.MenuID);
