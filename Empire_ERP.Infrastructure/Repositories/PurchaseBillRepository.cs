        ...
        public MyHttpResponseMessage GetPurchaseBillByCode(int code, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $"SELECT TRAN_ID,V_DATE,VOUCHER_NO,BTYPE, DOC,COMM,COMM_AMT,COMM_VAL,DISC,TERMS," +
        ...
        public MyHttpResponseMessage GetPurchaseBillDetailByCode(int code, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = "SELECT TRAN_ID,PARTY_CODE,ACT_CODE,DT_CODE,ITEM_CODE,HS_CODE," +
                ...
                if (!String.IsNullOrWhiteSpace(table) && !String.IsNullOrWhiteSpace(detailTable))
                {
                    ...
                    if (stk_status == "Y")
                    {
                        if (b_i == "B")
                        {
                            ...
                            string query = $@"EXEC STKPROC 71,'{startDate}','{endDate}','{common.Branch}','{common.Period}','','','','','{itemCodes}','','','B','','','',''";
                            using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                            {
                                SqlCommand command = new SqlCommand(query, connection);
                                connection.Open();
                            ...
                            try
                            {
                                ...
                                if (modelRecord.Master.TRAN_ID == null || modelRecord.Master.TRAN_ID == 0)
                                {
                                    ...
                                    query = $"INSERT INTO {table}" +
                                            "(TRAN_ID,V_DATE,VOUCHER_NO,PARTY_CODE,ACT_CODE,DOC,TERMS," +
                                            ...
                                            "VALUES" +
                                            "('" + code + "','" + modelRecord.Master.V_DATE + "','" + voucherNo + "','" + modelRecord.Master.PARTY_CODE + "','" + modelRecord.Master.ACT_CODE + "','" + modelRecord.Master.DOC + "','" + modelRecord.Master.TERMS + "'," +
                                            "'" + modelRecord.Master.REF + "','" + modelRecord.Master.CURR_CODE + "','" + modelRecord.Master.CRATE + "','" + modelRecord.Master.REMARKS + "','" + modelRecord.Master.SCODE + "','" + modelRecord.Master.SACODE + "','" + branch + "','" + period + "','" + modelRecord.Master.COMM + "','" + modelRecord.Master.COMM_VAL + "','" + modelRecord.Master.COMM_AMT + "','" + modelRecord.Master.DISC + "','" + modelRecord.Master.BTYPE + "'," +
            ...
            try
            {
                ...
                if (menuDetails.MD_ID == 17)
                {
                    topQuery = @$"SELECT  CM.C_NAME AS COMPANY_NAME,BR.B_ADDRESS,BR.B_TEL,BR.B_GST,BR.B_NTN,
                                 A.TRAN_ID , A.DISC,A.V_DATE, A.VOUCHER_NO ,
                ...
                else if (menuDetails.MD_ID == 18)
                {
                    ...
                    query = @$"SELECT  E.ITEM_NAME+' - '+E.REMARKS AS ITEM_NAME ,
                            SUM(B.QTY) AS QTY 
                              FROM {detailTable} B 
                             LEFT OUTER JOIN TBL_BARCODE BG
                             ON BG.CODE = B.ITEM_cODE
                            ...
                            AND B.DLT = 'T' 
                            Group By E.ITEM_ID,E.ITEM_NAME, E.REMARKS";
                }
                ...
                else
                {
                    ...
                    if (menuDetails.MD_ID == 17 || menuDetails.MD_ID == 18)
                    {
                        using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                        {
