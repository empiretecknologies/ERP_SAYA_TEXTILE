        ...
        public MyHttpResponseMessage GetReportData(SalesInvoiceReport report, Common common)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    {
                        string query = $"EXEC STKPROC '{report.ReportID}','{report.FromDate.Value.ToString("yyyy-MM-dd")}','{report.ToDate.Value.ToString("yyyy-MM-dd")}','{common.Branch}','{common.Period}','{report.Item}','{report.Group}','{report.Category}','{report.SubCategory}','{report.Barcode}','{report.Size}','{report.Color}','{b_i}','{report.Lot}','{report.PartyCode}','{report.ActCode}','{report.Branch}'";
                        SqlCommand command = new SqlCommand(query, connection);
                        connection.Open();
                        SqlDataReader reader = command.ExecuteReader();
