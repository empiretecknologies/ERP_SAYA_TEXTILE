...
{
    public class VendorPricingRepository : IVendorPricingRepository
    {
        public IBranchRepository _branchRepository { get; set; }
        public IMenuService _menuRepository { get; set; }
        public VendorPricingRepository(IBranchRepository branchRepository, IMenuService menuRepository)
        {
			...
			try
			{
				...
				using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
				{
                    string query = $@"SELECT A.TRAN_ID, A.V_DATE, A.VOUCHER_NO, A.CLIENT_PO, A.REF, A.REMARKS, A.PARTY_CODE, A.ACT_CODE, PT.PARTY_NAME, A.ADD_USER_ID, A.ADD_DATE, 
                                        ...
                                        FROM {table} A 
                                        LEFT OUTER JOIN {pickMaster} MPO ON MPO.PARTY_CODE = A.PARTY_CODE AND MPO.ACT_CODE = A.ACT_CODE AND MPO.CLIENT_PO = A.CLIENT_PO
        ...
        public MyHttpResponseMessage Save(CustomVendorPricing modelRecord, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                    ...
                    try
                    {
                        ...
                        if (modelRecord.Master.TRAN_ID == null || modelRecord.Master.TRAN_ID == 0)
                        {
                            ...
                            query = $"INSERT INTO {table} " +
                                    ...
                                    "EDIT_IP_ADDRESS, ADD_POSTALCODE, EDIT_POSTALCODE, ASTATUS, PICK_ID,JOB_NO, " +
                                    "MENU_ID, DLT) " +
                                    $"VALUES " +
                                    ...
                                    $"'{modelRecord.Master.ASTATUS}', '{modelRecord.Master.PICK_ID}', '{modelRecord.Master.JOBNO}', '{menuID}', 'T')";
                                ...
                                if (item.DT_CODE == null || item.DT_CODE == 0)
                                {
                                    ...
                                    if (detailCode > 0)
                                    {
                                        detailQuery = $@"INSERT INTO {detailTable}
                                                       (TRAN_ID, DT_CODE, SPARTY_CODE, SACT_CODE, YARN_CODE, COMM_RATE, RATE, PICK_ID, REV_STATUS, REV_REF,
                                                       ...
                                                       VALUES
                                                       ('{modelRecord.Master.TRAN_ID}', '{detailCode}', '{item.SPARTY_CODE}', '{item.SACT_CODE}', '{item.YARN_CODE}', '{item.COMM_RATE}',
                                                        '{item.RATE}','{modelRecord.Master.PICK_ID}','{item.REV_STATUS}',{detailCode},'{branch}', '{periodID}', '{username}', '{CommonService.GetDateTime("Pakistan Standard Time")}', 
                                ...
                                else if ((item.DT_CODE != null || item.DT_CODE != 0) && item.REV_TOGGLE == true)
                                {
                                    ...
                                    if (detailCode > 0)
                                    {
                                        detailQuery = $@"INSERT INTO {detailTable}
                                                       ...
                                                       VALUES
                                                       ('{modelRecord.Master.TRAN_ID}', '{detailCode}', '{item.SPARTY_CODE}', '{item.SACT_CODE}', '{item.YARN_CODE}', '{item.COMM_RATE}',
                                                        '{item.RATE}','{modelRecord.Master.PICK_ID}','{item.REV_STATUS}',{item.REV_REF},'{branch}', '{periodID}', '{username}', '{CommonService.GetDateTime("Pakistan Standard Time")}', 
                                                        '{computerName}', '{ip}', '{username}', '{CommonService.GetDateTime("Pakistan Standard Time")}', '{computerName}', '{ip}', '{postalCode}',
                                                        '{postalCode}', '{menuID}', 'T')";
        ...
        public MyHttpResponseMessage GetVendorPricingByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    string query = $@"SELECT VP.TRAN_ID, VP.V_DATE, VP.VOUCHER_NO, PM.VOUCHER_NO AS P_VOUCHER_NO, VP.PARTY_CODE, VP.ACT_CODE, VP.CLIENT_PO, 
                            VP.REF, VP.REMARKS, VP.ASTATUS,VP.JOB_NO
                            FROM {table} VP
                            LEFT OUTER JOIN {pickMaster} PM ON PM.TRAN_ID = VP.PICK_ID
                    ...
                    if (reader.Read())
                    {
                        var row = new
                        {
                            ...
                            PVOUCHER_NO = Convert.ToString(reader["P_VOUCHER_NO"]),
                            PARTY_CODE = $"{Convert.ToString(reader["PARTY_CODE"])}{Convert.ToString(reader["ACT_CODE"])}",
                            CLIENT_PO = Convert.ToString(reader["CLIENT_PO"]),
        ...
        public MyHttpResponseMessage GetVendorPricingDetailByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetBatchDetailByProcess(VendorPricing model, Common common)
        {
            ...
            try
            {
                ...
                string? table = string.Empty, detailTable = string.Empty, pickMasterTable = string.Empty, tranId = string.Empty, voucherNo = string.Empty;
                if (Menu.data != null)
                {
                ...
                if (!String.IsNullOrWhiteSpace(table) && !String.IsNullOrWhiteSpace(detailTable) && !String.IsNullOrWhiteSpace(pickMasterTable))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $@"SELECT TRAN_ID, VOUCHER_NO FROM {pickMasterTable} WHERE CLIENT_PO = '{model.CLIENT_PO}' AND PARTY_CODE = '{model.PARTY_CODE}' AND ACT_CODE = '{model.ACT_CODE}' AND TRAN_ID = '{model.JOBNO}' AND DLT = 'T' AND ASTATUS = 'Y'";
                        ...
                        if (reader.Read())
                        {
                            tranId = reader["TRAN_ID"].ToString();
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                VendorPricing VendorPricing = new VendorPricing();
                List<VendorPricingDetail> VendorPricingDetailList = new List<VendorPricingDetail>();
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (reader.Read())
                    {
                        VendorPricing = new VendorPricing
                        {
                    ...
                    {
                        var row = new VendorPricingDetail
                        {
                        ...
                        VendorPricingDetailList.Add(row);
                    }
                ...
                var customVendorPricing = new CustomVendorPricing
                {
                    Master = VendorPricing,
                    Detail = VendorPricingDetailList
                };

                response = this.Save(customVendorPricing, common, menu);
        ...
        public MyHttpResponseMessage DeleteVendorPricingDetailByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetDataForReport(VendorPricingRDLCReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            VendorPricingRDLCReport masterData = new VendorPricingRDLCReport();
            CustomVendorPricingForPrintReport reportData = new CustomVendorPricingForPrintReport();
            var Menu = _menuRepository.GetMenu(common.MenuID);
            ...
            try
            {
                ...
                topQuery = $@"SELECT M.TRAN_ID, M.V_DATE, M.VOUCHER_NO, MPO.CLIENT_PO, PC.PARTY_NAME AS CLIENT_NAME,
                                C.DESCR AS CURRENCY, PM.JOB_NO, M.REF, M.REMARKS, M.EDIT_USER_ID
                                FROM {table} M
                                LEFT OUTER JOIN {detailTable} D ON D.TRAN_ID = M.TRAN_ID
                                LEFT OUTER JOIN {pickTable} PM ON PM.TRAN_ID = M.JOB_NO
                                LEFT OUTER JOIN TBL_SP_MASTER MPO ON MPO.TRAN_ID = M.JOB_NO
