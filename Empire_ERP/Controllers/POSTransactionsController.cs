            ...
            try
            {
                ...
                if (modelRecord.Master.BILL_STATUS == "K" || modelRecord.Master.BILL_STATUS == "P")
                {
                    try
                    {
                        ...
                        if (modelRecord.Master.BILL_STATUS == "K" && modelRecord.Master.PWINDOW == 0)
                        {
                            ...
                            string pdfFileName = viewModel != null && !string.IsNullOrEmpty(viewModel.ReferenceNumber)
                                ? $"KOT_{model.tranId}_{viewModel.ReferenceNumber}.pdf"
                                : $"KOT_{model.tranId}_{DateTime.Now:yyyyMMddHHmmss}.pdf";
                            pdfFullPath = Path.Combine(slipFolder, pdfFileName);
                            PdfService.GenerateSlipPdf(slip, pdfFullPath, _env.WebRootPath);
                            StickerPath = PdfService.GenerateStickers(slip, pdfFullPath, _env.WebRootPath);
                        }
                        data.BarFilePath = StickerPath;
                        data.SlipFilePath = pdfFullPath;
                        data.PWindow = slip.PWindow;
                        data.KotPrinter = slip.KotPrinter;
                        data.StickerPrinter = slip.StickerPrinter;
