...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class PriceQuotedController : BaseController
    {
        public IPriceQuotedService _PriceQuotedService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public PriceQuotedController(IPriceQuotedService PriceQuotedService, IMenuService menuService, IWebHostEnvironment hostingEnvironment,IBaseService baseService) : base(menuService,baseService)
        {
            _PriceQuotedService = PriceQuotedService;
        ...
        [HttpGet]
        public JsonResult GetPriceQuoteds()
        {
            try
            {
                var data = _PriceQuotedService.QuickSearch(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult Save(CustomPriceQuoted modelRecord)
        {
            try
            {
                var data = _PriceQuotedService.Save(modelRecord, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetPriceQuotedByCode(int code)
        {
            try
            {
                var data = _PriceQuotedService.GetPriceQuotedByCode(code, CommonHelper.GetValues(HttpContext));
                var detailData = _PriceQuotedService.GetPriceQuotedDetailByCode(code, CommonHelper.GetValues(HttpContext));
        ...
        [HttpGet]
        public JsonResult GetPriceQuotedDetailByCode(int code)
        {
            try
            {
                var data = _PriceQuotedService.GetPriceQuotedDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetBatchDetailByProcess(PriceQuoted model)
        {
            try
            {
                var data = _PriceQuotedService.GetBatchDetailByProcess(model,  CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int code)
        {
            try
            {
                var data = _PriceQuotedService.Delete(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult CopyRecord(CopyRecord record)
        {
            try
            {
                var data = _PriceQuotedService.CopyRecord(record, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult DeletePriceQuotedDetailByCode(int code)
        {
            try
            {
                var data = _PriceQuotedService.DeletePriceQuotedDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetPrintReport(PriceQuotedRDLCReport model)
        {
        ...
        private string GenerateReport(PriceQuotedRDLCReport model)
        {
            var filePath = "";
            try
            {
                if (model != null && model.TRAN_ID > 0 && model.MD_ID > 0)
                {
                    ...
                    var responseMessage = _PriceQuotedService.GetDataForReport(model, reportDetails, CommonHelper.GetValues(HttpContext));
                    if (responseMessage.msgType != 1)
                    {
                        return "";
                    }
                    var reportData = (CustomPriceQuotedForPrintReport)responseMessage.data;
                    ...
                    using (LocalReport report = new LocalReport())
                    {
                        ...
                        using (var stReader = new StreamReader(path))
                        {
                            ...
                            using (var stream = new MemoryStream(byteArray))
                            {
                                ...
                                byte[] file;
                                string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, @"Client\PriceQuoted");
                                ...
                                if (report.IsReadyForRendering)
                                {
                                    ...
                                    filePath = $"/Client/PriceQuoted/{filePath}";
                                }
                            }
                        }
                    }
        ...
        public async Task<IActionResult> SaveImage()
            ...
            try
            {
                ...
                {
                    string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, "Client", "uploads", "PriceQuoted");
                    ...
                    response.msgType = 1;
                    response.data = $"/Client/uploads/PriceQuoted/{uniqueFileName}"; // Return file path
                }
                else
                {
