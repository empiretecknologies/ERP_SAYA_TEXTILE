...
namespace Empire_ERP.Controllers
{
    [ExtractMenuCode]
    [CheckSession]
    public class PurchaseOrderReportsController : BaseController
    {
        public IPeriodService _periodService { get; set; }
        public IPurchaseOrderReportsService _PurchaseOrderReportservice { get; set; }
        public ICompanyService _companyService { get; set; }
        public PurchaseOrderReportsController(IMenuService menuService, IPeriodService periodService, IPurchaseOrderReportsService PurchaseOrderReportservice, ICompanyService companyService ,IBaseService baseService) : base(menuService,baseService)
        {
        ...
        public JsonResult GetReportTypes()
        {
            try
            {
                var data = _PurchaseOrderReportservice.GetReportTypes(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GenerateReport(PurchaseOrderReports report)
        {
            try
            {
                var common = CommonHelper.GetValues(HttpContext);
                ...
                var data = _PurchaseOrderReportservice.GetReportData(report, CommonHelper.GetValues(HttpContext), menu);
                return Json(data);
            }
            catch (Exception ex)
            {
