...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class SampleDevAndPricingController_OLD : BaseController
    {
        public ISampleDevAndPricingService_OLD _SampleDevAndPricingService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public IPeriodService _periodService { get; set; }
        public SampleDevAndPricingController_OLD(ISampleDevAndPricingService_OLD SampleDevAndPricingService, IMenuService menuService, IWebHostEnvironment hostingEnvironment, IPeriodService periodService,IBaseService baseService) : base(menuService,baseService)
        {
        ...
        //{
        //    var data = _SampleDevAndPricingService.GetDeliveryFormats(CommonHelper.GetValues(HttpContext));
        //    return Json(data);
        //}
        ...
        //    try
        //    {
        //        var data = _SampleDevAndPricingService.GetAccountsForTreeView(CommonHelper.GetValues(HttpContext));
        //        return Json(data);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        //    try
        //    {  
        //        var data = _SampleDevAndPricingService.QuickSearch(CommonHelper.GetValues(HttpContext), skip, take, filter, group);
        //        return Json(data);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        [HttpGet]
        public JsonResult SampleDevAndPricingByid(int id)
        {
            var data = _SampleDevAndPricingService.GetSampleDevAndPricingById(id, CommonHelper.GetValues(HttpContext));
            return Json(data);
        }
        ...
        [HttpPost]
        public JsonResult Save(SampleDevAndPricing_OLD model)
        {
            try
            {
                var data = _SampleDevAndPricingService.Save(model, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                var data = _SampleDevAndPricingService.Delete(id, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetPrintReport(SampleDevAndPricingReport model)
        {
        ...
        private string GenerateReport(SampleDevAndPricingReport model)
        {
            var filePath = "";
            try
            {
                if (model != null && model.TRAN_ID > 0 && model.MD_ID > 0)
                {
                    ...
                    var responseMessage = _SampleDevAndPricingService.GetDataForReport(model, CommonHelper.GetValues(HttpContext));
                    if (responseMessage.msgType != 1)
                    {
                        return "";
                    }
                    var reportData = (SampleDevAndPricingReport)responseMessage.data;
