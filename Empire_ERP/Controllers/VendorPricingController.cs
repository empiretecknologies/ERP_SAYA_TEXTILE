...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class VendorPricingController : BaseController
    {
        public IVendorPricingService _VendorPricingService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public VendorPricingController(IVendorPricingService VendorPricingService, IMenuService menuService, IWebHostEnvironment hostingEnvironment,IBaseService baseService) : base(menuService,baseService)
        {
        ...
        [HttpGet]
        public JsonResult GetVendorPricings()
        {
            try
            {
                var data = _VendorPricingService.QuickSearch(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult Save(CustomVendorPricing modelRecord)
        {
            try
            {
                var data = _VendorPricingService.Save(modelRecord, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetVendorPricingByCode(int code)
        {
            try
            {
                var data = _VendorPricingService.GetVendorPricingByCode(code, CommonHelper.GetValues(HttpContext));
                var detailData = _VendorPricingService.GetVendorPricingDetailByCode(code, CommonHelper.GetValues(HttpContext));
        ...
        [HttpGet]
        public JsonResult GetVendorPricingDetailByCode(int code)
        {
            try
            {
                var data = _VendorPricingService.GetVendorPricingDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetBatchDetailByProcess(VendorPricing model)
        {
            try
            {
                var data = _VendorPricingService.GetBatchDetailByProcess(model,  CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int code)
        {
            try
            {
                var data = _VendorPricingService.Delete(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult CopyRecord(CopyRecord record)
        {
            try
            {
                var data = _VendorPricingService.CopyRecord(record, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult DeleteVendorPricingDetailByCode(int code)
        {
            try
            {
                var data = _VendorPricingService.DeleteVendorPricingDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetPrintReport(VendorPricingRDLCReport model)
        {
        ...
        private string GenerateReport(VendorPricingRDLCReport model)
        {
            var filePath = "";
            try
            {
                if (model != null && model.TRAN_ID > 0 && model.MD_ID > 0)
                {
                    ...
                    var responseMessage = _VendorPricingService.GetDataForReport(model, reportDetails, CommonHelper.GetValues(HttpContext));
                    if (responseMessage.msgType != 1)
                    {
                        return "";
                    }
                    var reportData = (CustomVendorPricingForPrintReport)responseMessage.data;
                    ...
                    using (LocalReport report = new LocalReport())
                    {
                        ...
                        using (var stReader = new StreamReader(path))
                        {
                            ...
                            using (var stream = new MemoryStream(byteArray))
                            {
                                ...
                                byte[] file;
                                string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, @"Client\VendorPricing");
                                ...
                                if (report.IsReadyForRendering)
                                {
                                    ...
                                    filePath = $"/Client/VendorPricing/{filePath}";
                                }
                            }
                        }
                    }
        ...
        public async Task<IActionResult> SaveImage()
            ...
            try
            {
                ...
                {
                    string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, "Client", "uploads", "VendorPricing");
                    ...
                    response.msgType = 1;
                    response.data = $"/Client/uploads/VendorPricing/{uniqueFileName}"; // Return file path
                }
                else
                {
