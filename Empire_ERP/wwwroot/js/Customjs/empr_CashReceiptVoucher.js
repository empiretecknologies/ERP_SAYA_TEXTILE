var empr_CashReceiptVoucher = {
    ...
    ResetForm: function () {
        empr_CashReceiptVoucher.CreateGrid([{ __KEY__: empr_CashReceiptVoucher.GenerateKey(36), dC_TYPE: empr_CashReceiptVoucher.DC_TYPE, chQ_DATE: todayDate, curR_CODE: 1, crate: 1 }]);
        ...
        var col = [
            ...
            },
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    if (filtered.length > 0) {
                        ...
                        var rate = parseFloat((parseFloat(newData.crate) || 0).toFixed(2));
                        var amt = parseFloat((parseFloat(currentRowData.amt) || 0).toFixed(2));

                        newData.pkR_AMT = (rate * amt) || 0;

                    } else {
                        newData.curR_CODE = null;
                        newData.crate = null;
                        newData.pkR_AMT = null;
                    }
                }
            },
            {
            ...
            },
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    var rate = parseFloat((parseFloat(currentRowData.crate) || 0).toFixed(2));
                    var amt = parseFloat((parseFloat(newData.amt) || 0).toFixed(2));
    ...
    AddRow: function () {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                dataSource.unshift({ __KEY__: empr_CashReceiptVoucher.GenerateKey(36), dC_TYPE: empr_CashReceiptVoucher.DC_TYPE, curR_CODE: 1, crate: 1 });
    ...
    GeneratePrintReport: function () {
        ...
        ajaxHelper.ajaxPostJsonData(dataModel, "/CashReceiptVoucher/GetPrintReport", function (data) {
            if (data.msgType == 1) {
                ...
                // window remove
                var reportUrl = window.location.origin + data.data;
                $('#hiddenPrintIframe').remove();
                var $iframe = $('<iframe id="hiddenPrintIframe" style="display:none;"></iframe>');
                $('body').append($iframe);
                ...
                $iframe.on('load', function () {
                    try {
                        ...
                        var printWin = window.open(reportUrl, '_blank');
                        if (printWin) {
                            printWin.print();
                        }
                    }
    ...
    ShowKnockOffModal: function (tranId, dtCode, amt, partyCode, actCode, partyName, bookName, whtRate, dcType) {
        var SumDetailCharges = 0;

        ajaxHelper.ajaxGetJson('/CashReceiptVoucher/GetPartyCurrentBalance?partyCode=' + partyCode + '&actCode=' + actCode, function (data) {
            console.log('partyBalance', data);

            let partyBalance = data.filter(x =>
                String(x.partyCode) === String(partyCode) &&
                String(x.accountCode) === String(actCode)
            );
            ...
            debugger;
            if (data.length > 0) {
                balance = partyBalance[0].balance
            }
            else {
                empr_helper.notify('Getting error while fetching party balance',2);
            }
            ...
            $('#DetailChargesGridContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                ajaxHelper.ajaxPostJsonData({ modelRecord: obj }, "/CashReceiptVoucher/SaveDetailCharges", function (data) {
                    if (data.msgType == 1) {
                        empr_helper.notify(data.msg, data.msgType);
                        empr_CashReceiptVoucher.GetDetailCharges(P_TRAN_ID, P_DT_CODE);
                        empr_KnockOff.SetValuesAfterSaveCharges(P_TRAN_ID, P_DT_CODE);
                    }
                }, false, true);
            }).fail(function () {
                console.error("Failed to save changes.");
        ...
        if (dtCode == '' || dtCode == null || dtCode == undefined) {
            ...
            ajaxHelper.ajaxPostJsonData(obj, "/CashReceiptVoucher/DeleteDetailCharges", function (data) {
                if (data.msgType == 1) {
                    ...
                    empr_KnockOff.SetValuesAfterSaveCharges(tranId, dtCode);
                }
            }, false, true);
        }
