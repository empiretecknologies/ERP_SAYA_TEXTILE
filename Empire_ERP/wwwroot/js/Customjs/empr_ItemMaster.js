var empr_ItemMaster = {
    ...
    initEvents: function () {
        $(document).ready(function () {
            ...
            empr_ItemMaster.InitItemSizesDDL();
            ...
            $('#SizeBtnSave').click(function () {
                empr_ItemMaster.SizeSaveAttempt();
            });
            ...
            //$('body').on('click', '.elm_editItemSizesInfo', function () {
            //    var reportid = $(this).attr("reportid")
            //    var code = $('#Code').val();
            //    empr_ItemMaster.GetBarcodeInfoById(reportid, code);
            //});
    ...
    GetItemMasterByCode: function (id) {
        ...
        ajaxHelper.ajaxGetJson('/ItemMaster/GetItemMasterByCode?code=' + id, function (data) {
            ...
            if (data.mainData.msgType == 1) {
                ...
                //}
                empr_ItemMaster.GetItemSizesInfo();
                $('.modal').modal('hide');
            }
            else {
    ...
    InitItemSizedGrid: function (dataSrc) {
        var col = [
            {
                dataField: "Action",
                width: 100,
                ...
                cellTemplate: function (container, options) {
                    ...
                    $(`<div class="btn-group btn-group-sm">
                            <a href="javascript:;"  class="grid-action-icon elm_editItemSizesInfo" reportid=${options.data.id} title="Edit"><i class="fa fa-edit"></i></a>
                            <a href="javascript:;"  class="grid-action-icon Delete" style = "margin-left: 8px" onclick = "empr_ItemMaster.DeleteItemSizesInfo(${options.data.id})" title = "Delete" > <i class="fa fa-trash"></i></a>
        ...
        empr_helper.dxGridbindingVouchers('#itemSizesGridContainer', col, dataSrc, "itemSizesInfo");
    },
    ...
    //    return modelRecord;
    //},
    DeleteItemSizesInfo: function (id) {
        console.log(id);
        debugger;
        if (!id) {
        ...
        code = $('#Code').val();
        var xhr = ajaxHelper.ajaxPostJsonData(null, "/ItemMaster/DeleteItemSizeInfo?sizeId=" + id + "&sizeCode=" + code, function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                empr_ItemMaster.GetItemSizesInfo();
                //$('#BtnDeleteItemSizes').hide();
                //empr_ItemMaster.InitBarcodeInfoForm();
                //empr_ItemMaster.GetBarcodeInfo();
                //$("#BarCodeType").dxSelectBox("instance").option("disabled", false);
            }
    ...
    //},
    GetItemSizesInfo: function () {

        var code = $('#Code').val();
        if (code > 0) {
            ajaxHelper.ajaxGetJson('/ItemMaster/GetItemSizesInfoByCode?code=' + code, function (data) {

                debugger;
                empr_ItemMaster.InitItemSizedGrid(data.data);
    ...
    SizeSaveAttempt: function () {
        debugger;
        var dataModel = {
            ITEM_CODE: $('#Code').val(),
            ITEM_SIZE: $("#ITEM_SIZE").dxSelectBox('instance').option('value'),
        };

        ajaxHelper.ajaxPostJsonData(dataModel, "/ItemMaster/SaveItemSize", function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                empr_ItemMaster.GetItemSizesInfo();
            }
    ...
    resetForm: function () {
        ...
        //$('#AttributeBtnDelete').hide();
        $('#BtnNew').hide();
        empr_ItemMaster.InitItemSizedGrid();
        $('#pills-itemsizes-tab').removeClass('active');
    ...
    InitItemSizesDDL: function (selectedValue) {

        $('#ITEM_SIZE').dxSelectBox({
            dataSource: Sizes,
            displayExpr: 'value',
            valueExpr: 'key',
            value: selectedValue,
