var empr_KnockOff = {
    SumDetailCharges: PickData.chargeS_SUM,
    voucherAmount: PickData.amount,
    ...
    initEvents: function () {
        $(document).ready(function () {
            ...
            empr_KnockOff.SetRemainingAmount(empr_KnockOff.voucherAmount, empr_KnockOff.SumDetailCharges);
            empr_KnockOff.SetPartyBalance(PickData.partY_BAL);


            $("#ko_amt_span").text((empr_KnockOff.voucherAmount + empr_KnockOff.SumDetailCharges).toLocaleString());
            ...
            $('#showDetailModal').click(function () {
                empr_CashReceiptVoucher.DetailCharges(PickData.traN_ID, PickData.dT_CODE);
    ...
    SetValuesAfterSaveCharges: function (tranId, dtCode) {
        ajaxHelper.ajaxGetJson('/CashReceiptVoucher/GetSumDetailCharges?tranId=' + tranId + '&dtCode=' + dtCode, function (data) {
            debugger;
            if (data.msgType == 1) {
                empr_KnockOff.SumDetailCharges = data.data;
                $("#ko_amt_span").text((empr_KnockOff.voucherAmount + data.data).toLocaleString());
                empr_KnockOff.SetRemainingAmount(empr_KnockOff.voucherAmount, empr_KnockOff.SumDetailCharges);
                $("#P_AMOUNT").val(empr_KnockOff.voucherAmount + empr_KnockOff.SumDetailCharges);
            }
            else {
    ...
    SetRemainingAmount: function (amount, chargesSum) {
        debugger;
        ...
        var RemainingAmount = (amount + chargesSum) - totalKO;
    ...
    SetPartyBalance: function (partyBal) {
        let numericBal = parseFloat(partyBal) || 0;

        let formattedBal = numericBal.toLocaleString('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
        });

        let element = $("#PARTY_BAL");
        element.text(formattedBal);
        ...
        var col = [
            ...
            },
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    var wht = parseFloat(currentRowData.wht) || 0;
                    var whtAmt = IntValue * wht / 100 || 0;

                    //if (PickData.dC_TYPE == 'D') {
                    //    var netAmt = IntValue + whtAmt;
                    //} else if (PickData.dC_TYPE == 'C') {
                    //    var netAmt = IntValue - whtAmt;
                    //}
            ...
            },
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    newData.wht = value;
                    var IntValue = parseFloat(value) || 0;
                    var kO_AMT = parseFloat(currentRowData.kO_AMT) || 0;
                    var whtAmt = IntValue * kO_AMT / 100 || 0;

                    var netAmt = kO_AMT - whtAmt;
                    newData.whT_AMT = whtAmt.toFixed(2);
                    newData.whT_NET_AMT = netAmt.toFixed(2);
                    setTimeout(function () {
                        empr_KnockOff.SetRemainingAmount(empr_KnockOff.voucherAmount, empr_KnockOff.SumDetailCharges);
            ...
            },
            {
                ...
                //allowEditing: false,
                setCellValue: function (newData, value, currentRowData) {
                    newData.whT_AMT = value;
                    var whtAmt = parseFloat(value) || 0;
                    var kO_AMT = parseFloat(currentRowData.kO_AMT) || 0;

                    var whtPercent = 0;
                    if (kO_AMT > 0) {
                        whtPercent = (whtAmt / kO_AMT) * 100;
                    }

                    var netAmt = kO_AMT - whtAmt;

                    newData.wht = whtPercent.toFixed(2);
                    newData.whT_NET_AMT = netAmt.toFixed(2);
    ...
    AddRow: function (selectedRow) {
        ...
        //if (PickData.dC_TYPE == 'D') {
        //    netAmt = selectedRow.amount + whtAmt;
        //} else if (PickData.dC_TYPE == 'C') {
        //    netAmt = selectedRow.amount - whtAmt;
        //}
