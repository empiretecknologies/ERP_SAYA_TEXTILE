var empr_MerchantPurchaseOrder = {
    ...
    initEvents: function () {
        ...
        $(document).ready(function () {
            ...
            $('#AMT').on('input', function () {
                ...
                empr_MerchantPurchaseOrder.calculateNetAmt();
            });
    ...
    calculateNetAmt: function () {
        debugger;
        var OTHER_INCOME = parseFloat($("#OTHER_INCOME").val()) || 0;
        var COMM_VAL = parseFloat($("#COMM_VAL").val()) || 0;
        var SUM_AMT = OTHER_INCOME + COMM_VAL;
        $("#NET_AMT").val(SUM_AMT);
    },
    ...
    GetMerchantPurchaseOrderByID: function (id) {
        var xhr = ajaxHelper.ajaxGetJson('/MerchantPurchaseOrder/MerchantPurchaseOrderByid?id=' + id, function (data) {
            ...
            if (data.msgType == 1) {
                ...
                $("#OTHER_INCOME").val(data.data.otheR_INCOME);
                $("#NET_AMT").val(data.data.neT_AMT);
    ...
    InitPayTermsDDL: function (dataSource, selectedValue) {
        if (selectedValue == undefined || selectedValue == null) {
            selectedValue = dataSource[0].key;
        }
        $('#TERMS').dxSelectBox({
            dataSource: {
                store: dataSource,
