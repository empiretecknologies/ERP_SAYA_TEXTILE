...
var empr_PurchaseBill = {
    ...
    InitEvents: function () {
        $(document).ready(function () {
            console.log('Pick Type', typeForm);
            ...
            ajaxHelper.ajaxGetJson('/PurchaseBill/GetBarcodeList', function (data) {
                ...
                if (data.msgType == 1) {
                    if (data.data.length > 0) {
                        ...
                        //}
                        empr_PurchaseBill.CreateBarcodeGrid(data.data);
    ...
    CreateGrid: function (dataSrc) {
        ...
        var col = [
            ...
            },
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    //debugger;
                    console.log(" item daata ", newData)
                    ...
                    if (Type == "I") {
                        if (value && Items && Items.length > 0) {
                            ...
                            var selectedItem = Items.filter(
                                u => String(u.key) === String(value)
                            );

                            if (selectedItem.length > 0) {
                                newData.rate = selectedItem[0].rate;
                                newData.hS_CODE = selectedItem[0].hscode;
                            }
                        }
                    }
                }
        ...
        if (Type === "I") {
            col.push(
                ...
                },
                {
                    ...
                    allowEditing: false,
                    //calculateCellValue: function (data) {
                    //    var BalanceQty = parseFloat(data.baL_QTY) || 0;
                    //    var Rate = parseFloat(data.rate) || 0;
                    //    var Amount = BalanceQty * Rate || 0;
                    //    var Discount = parseFloat(data.disc) || 0;
                    //    return data.disC_AMT = (Amount * Discount / 100).toFixed(2);
                    //}
                },
                {
                    dataField: 'tax',
            ...
            col.push(
                ...
                },
                {
                    ...
                    setCellValue: function (newData, value, currentRowData) {
                        newData.rate = value;
                        updateAmounts(newData, currentRowData);
                        var rate = parseFloat(newData.rate) || 0;
                        var qty = parseFloat(currentRowData.baL_QTY) || 0;
                        if (!isNaN(qty) && !isNaN(rate)) {
                            newData.amt = (qty * rate).toFixed(2);
        ...
        ajaxHelper.ajaxGetJson('/PurchaseBill/GetPurchaseBillByCode?code=' + code, function (data) {
            ...
            if (data.master.msgType == 1) {
                ...
                if (data.detail.msgType == 1) {
                    empr_PurchaseBill.CreateGrid(data.detail.data);
                }
                else {
                    empr_helper.notify(data.msg, data.msgType);
                }
    ...
    ValidateMainInfo: function () {
        ...
        $.each(data.Detail, function (index, item) {
            if (item.iteM_CODE == "" || item.iteM_CODE == null || item.iteM_CODE == undefined) {
                empr_helper.notify("Please select Barcode at index " + index, 2);
                valid = false;
                return valid;
