var empr_helper = {
    ...
    SimpleEditableGrid: function (div, columns, datasrc, selectionMode, height) {
        ...
        const dataGrid = $(div).dxDataGrid({
            ...
            onEditingStart: function (e) {
                if (e.column.dataField === "fab" || e.column.dataField === "sti" || e.column.dataField === "acc") {

                    if (e.data && e.data.isNewRow) {
                        e.cancel = false;
                    } else {
                        e.cancel = true;
                    }
            ...
            onEditorPreparing: function (e) {
                if (e.parentType === "dataRow" && ['f_MIN', 'f_MAJ', 's_MIN', 's_MAJ', 'a_MIN', 'a_MAJ'].includes(e.dataField)) {
                    e.editorOptions.onValueChanged = function (args) {
                        // 1. Cell ki value update karein
                        e.setValue(args.value);

                        // 2. Apna footer wala method call karein
                        // setTimeout isliye taaki DevExtreme pehle internal state update karle
                        setTimeout(function () {
            ...
            onCellPrepared: function (e) {
                if (e.rowType === "data") {
                    ...
                    if (e.column.dataField === "f_MIN" || e.column.dataField === "s_MIN" || e.column.dataField === "a_MIN") {
                        e.cellElement.css({
                            "background-color": "rgb(17 189 29 / 75%)",
                            "color": "rgb(0 0 0)"
                        });
                    }

                    if (e.column.dataField === "f_MAJ" || e.column.dataField === "s_MAJ" || e.column.dataField === "a_MAJ") {
                        e.cellElement.css({
                            "background-color": "rgb(244 0 36 / 69%)",
                            "color": "rgb(0 0 0)"
                        });
                    }

                    if (e.column.dataField === "miN_DEF") {
                        e.cellElement.css({
                            "background-color": "rgb(17 189 29 / 75%)",
                            "color": "rgb(0 0 0)",
                            "font-weight": "bold"
    ...
    dxGridLock: function (div, columns, datasrc, fileName, selectionMode, extraOptions, height) {
        let selection;
        ...
        const dataGrid = $(div).dxDataGrid({
            ...
            //    if (e.rowType === "data") {
            //        var diff = e.data.diff_per || 0;
            //        //console.log('decreasePercentage', diff);
            //        if (diff > 3) {
            //            //e.rowElement.removeClass("danger-row");
            //            e.rowElement.addClass("danger-row");
            //        } else {
            //            //e.rowElement.addClass("danger-row");
            //            e.rowElement.removeClass("danger-row");
            //        }
            ...
            onRowPrepared: function (e) {
                debugger;
                if (e.rowType === "data") {
                    // Agar diff_per DB se nahi aa raha, to yahin calculate karlein
                    var originalQty = parseFloat(e.data.qty) || 0;
                    var offerQty = parseFloat(e.data.offeR_QTY) || 0;

                    var diff = 0;
                    if (originalQty > 0) {
                        diff = ((originalQty - offerQty) / originalQty) * 100;
                    } else {
                        diff = e.data.diff_per || 0;
                    }
            ...
            //onRowPrepared: function (e) {
            //    if (e.rowType === "data") {
            //        var dtCode = e.data.dT_CODE;

            //        var isValidDtCode = dtCode !== null &&
            //            dtCode !== undefined &&
            //            dtCode !== "" &&
            //            dtCode !== 0 &&
            //            dtCode !== "null";
            ...
            //            }
            //        } else {
            //            // Agar dT_CODE invalid hai to red color hata do
            //            e.rowElement.removeClass("danger-row");
            //        }
            //    }
            //},
            onEditorPreparing: function (e) {
                if (e.parentType === "dataRow" && ['offeR_QTY',].includes(e.dataField)) {
                    e.editorOptions.onValueChanged = function (args) {
                        e.setValue(args.value);
                        setTimeout(function () {
                            empr_InspectionProcess.updateFooterSums();
            ...
            onToolbarPreparing: function (e) {
                e.toolbarOptions.items = e.toolbarOptions.items.filter(function (item) {
                    // 'saveButton' aur 'revertButton' ko nikaal rahe hain
                    return item.name !== 'saveButton' && item.name !== 'revertButton';
                });
            },
    ...
    dxGridForQCForm: function (div, columns, datasrc, fileName, selectionMode, extraOptions, height) {
        debugger;
        let selection;
        ...
        const dataGrid = $(div).dxDataGrid({
            ...
            // ... baki settings
            "headerFilter": {
                "visible": true, // Isse header par filter icon (funnel) aa jayega
                "search": {
                    "enabled": false // Isse filter ki list ke andar wala search box band rahega
                }
            },
            "filterRow": {
                "visible": false, // Isay false rakhen, taake column ke niche wala box gayab ho jaye
            },
            // ... baki settings
            // ... baki settings
            editing: {
            ...
            onToolbarPreparing: function (e) {
                // 1. Pehle default buttons (save, revert, addRow) ko nikal den
                // Sirf 'searchPanel' ko rehne den
                e.toolbarOptions.items = e.toolbarOptions.items.filter(function (item) {
                    return item.name === 'searchPanel';
                });

                // 2. Ab apna 'Refresh' button add karein
        ...
        const dataGrid = $(div).dxDataGrid({
            ...
            onEditorPreparing: function (e) {
                if (e.dataType === "number") {
                    ...
                    //    // EnhancedDecimalFieldValidation logic
                    ...
                    //    }
                    //};
                    //e.editorOptions.onKeyPress = function (e) {
                    //    // EnhancedDecimalFieldValidation logic
                    //    var element = e.element;
                    //    var value = $(element).val();
                    //    if (e.key.length === 1 && /[^0-9.]/.test(e.key)) {
                ...
                //    if (e.dataField === "credit") {
                //        // Disable editing for editableColumn2
                //        e.editorOptions.disabled = true;
                //    } else if (e.dataField === "debit") {
                //        // Disable editing for editableColumn1
                //        e.editorOptions.disabled = true;
                //    }
                //}
                ...
                //if (e.parentType === "dataRow") {
                //    if (e.dataField === "credit") {
                //        // Disable editing for debit column if credit is being edited
                //        e.row.data.debit = 0;
                //        e.editorOptions.disabled = !!e.row.data.debit;
                //    } else if (e.dataField === "debit") {
                //        // Disable editing for credit column if debit is being edited
                //        e.row.data.credit = 0;
                //        e.editorOptions.disabled = !!e.row.data.credit;
                //    }
                //}

                //if (e.parentType === "dataRow" && (e.dataField === "credit" || e.dataField === "debit")) {
                //    e.editorOptions.onValueChanged = function (args) {
                //        if (args.value < 0) {
                //            args.component.option("value", 0); // Set value to 0 if it's negative
                //        }
                //    };
                //}
    ...
    editableDxGridbindingForTransactions: function (div, columns, datasrc, fileName, firstColumn, selectionMode, isEditable = true, height = 300) {
        ...
        const dataGrid = $(div).dxDataGrid({
            ...
            //    if (e.column.dataField === 'custoM_ACT_CODE') {
            //        setTimeout(function () {
            //            e.component.editCell(e.rowIndex, e.columnIndex);
            //            var cellElement = e.component.getCellElement(e.rowIndex, e.columnIndex);

            //            if (cellElement) {
            //                var $editorInput = $(cellElement).find('.dx-dropdowneditor-input');
            //                if ($editorInput.length > 0) {
            //                    $editorInput.trigger('dxclick');
            //                }
            //            }
            //        }, 1700);
            //    }
            ...
            onCellPrepared(e) {
                ...
                /*                if (column.caption && (column.caption.toLowerCase().includes('date') || column.caption.toLowerCase().includes('exp')) && (value == '1900-01-01' || value == '01-01-1900' || value == '01-Jan-1900' || value == '1/1/1900 12:00:00 AM' || value == '01/01/1900 12:00:00 AM' || value == '1/1/1900' ||
                    ...
                    value == '00-01-01' || value == '01-01-00' || value == '01-Jan-00' || value == '1/1/00 12:00:00 AM' || value == '01/01/00 12:00:00 AM' || value == '1/1/00')) {
                    console.log("In It")
                    // Set the value to undefined to indicate invalid/missing data
                    if (rowData && column.dataField) {
                        rowData[column.dataField] = undefined; // Update the data source
                    }
                }*/
            ...
            onEditorPreparing: function (e) {
                if (e.parentType === "dataRow" && e.row && e.row.data) {
                    var rowData = e.row.data;
                    var isPicked = rowData.picK_ID != null && rowData.picK_ID != 0 && rowData.picK_ID !== "";

                    // Supplier aur Yarn lock logic
                    //if (isPicked && (e.dataField === "supplieR_CODE" || e.dataField === "yarN_CODE")) {

                    //    e.editorOptions.readOnly = true;
                    //    e.editorOptions.disabled = true;
                    //}

                    // Cover No aur Insurance Date logic
                    if ((e.dataField === "coveR_NO" || e.dataField === "inS_DATE") && rowData.insrancE_STATUS) {
                        e.editorOptions.disabled = true;
                    }

                    // Rate logic
                    if (e.dataField === 'rate' && (typeof div !== 'undefined' && div === '#deliveryFeedingDetailContainer') && isPicked) {
                        e.editorOptions.readOnly = true;
                    }
                }

                // Baqi aapka Item Code aur Number validation logic yahan rahega...
                ...
                //        value = value.replace(/[^0-9.]/g, '').replace(/\.(?=.*\.)/g, '').replace(/(\.\d{3})\d+/g, '$1');

                //        if (value === '' || parseFloat(value) >= 0) {
                //            inputElement.val(value);
                //        } else {
                //            inputElement.val($(element).data('lastValid') || '');
                //        }
                //        $(element).data('lastValid', inputElement.val());
                //    };
                //}
            },
            onRowUpdating: function (e) {
    ...
    exportGridToPdf: function (gridComponent, isLandscape, customFontSize, CurrentReportName, isSubReport) {
        debugger;
        isLandscape = isLandscape !== undefined ? isLandscape : true;
        customFontSize = customFontSize || 9;
        const headerBgColor = '#055a87';

        const logoUrl = '/Client/Company/logoOri.png'; 
        const reportName = isSubReport ? 'Mpo Layout Summary' : CurrentReportName;
        const rawFromDate = $("#FROM_DATE").val();
        const fromDate = rawFromDate.split('-').reverse().join('-');
        const rawToDate = $("#TO_DATE").val();
        const toDate = rawToDate.split('-').reverse().join('-');
        const printedDate = '01-07-2026';
        const printedTime = '11:54 AM';
        const newTab = window.open('about:blank', '_blank');
        if (!newTab) {
            empr_helper.notify('Please allow popups for this website to view the PDF.', 2);
            return;
        }
        newTab.document.write('<title>Generating PDF...</title><h2 style="font-family:sans-serif; text-align:center; margin-top:20%;">Please wait, generating your report...</h2>');
        try {
            const doc = new jsPDF({
            ...
            Promise.all([
                fetch('/fonts/TCM_____.TTF').then(r => r.arrayBuffer()),
                fetch('/fonts/TW_BOLD.TTF').then(r => r.arrayBuffer()),
                fetch(logoUrl).then(r => r.blob()) 
            ]).then(([regularBuffer, boldBuffer, logoBlob]) => {
                ...
                // Fonts setup
                doc.addFileToVFS('TwCenMT.ttf', arrayBufferToBase64(regularBuffer));
                doc.addFont('TwCenMT.ttf', 'TwCenMT', 'normal');

                doc.addFileToVFS('TwCenMT-Bold.ttf', arrayBufferToBase64(boldBuffer));
                doc.addFont('TwCenMT-Bold.ttf', 'TwCenMT', 'bold');
                ...
                reader.onloadend = function () {
                    ...
                    const logoWidth = 40;
                    const logoHeight = 20;
                    doc.addImage(base64Logo, 'PNG', marginX, cursorY, logoWidth, logoHeight);

                    cursorY += logoHeight + 2;

                    doc.setFont('TwCenMT', 'bold');
                    doc.setFontSize(20);
                    doc.setTextColor(headerBgColor);
                    doc.text(reportName, pageWidth / 2, cursorY, { align: 'center' });
                    cursorY += 3;

                    const infoBarHeight = 6;
                    doc.setFillColor(headerBgColor);
                    doc.rect(marginX, cursorY, pageWidth - (marginX * 2), infoBarHeight, 'F');

                    doc.setFont('TwCenMT', 'normal');
                    doc.setFontSize(customFontSize + 1);
                    doc.setTextColor('#FFFFFF');
                    const infoY = cursorY + infoBarHeight - 2;

                    doc.text(`From : ${fromDate}`, marginX + 2, infoY);
                    doc.text(`To : ${toDate}`, pageWidth / 2 - 20, infoY);

                    cursorY += infoBarHeight + 4;

                    const tableStartY = cursorY;

                    // Reset state before autotable renders
                    ...
                    DevExpress.pdfExporter.exportDataGrid({
                        ...
                        topLeft: { x: marginX - 14.1, y: tableStartY - 17.5 },
                        ...
                        autoTableOptions: {
                            ...
                            margin: { top: tableStartY, left: marginX, right: marginX, bottom: 2 }
                        },
                        ...
                        customizeCell: function (options) {
                            ...
                            if (pdfCell.font) {
                                pdfCell.font.name = 'TwCenMT';
                                pdfCell.font.size = customFontSize;
                            ...
                            if (gridCell.rowType === 'data' && gridCell.column && gridCell.column.dataType === 'number') {
                                const numericValue = Number(gridCell.value);
                                if (gridCell.value === null || gridCell.value === undefined || gridCell.value === '' || numericValue === 0) {
                                    pdfCell.text = '';
                                } else if (numericValue < 0) {
                                    let formattedText = Array.isArray(pdfCell.text) ? pdfCell.text.join('') : pdfCell.text;
                                    formattedText = formattedText.replace('-', '').trim();
                                    pdfCell.text = `(${formattedText})`;
                                    pdfCell.textColor = '#FF0000';
                                }
                            }
                            if (gridCell.rowType === 'header') {
                                pdfCell.backgroundColor = headerBgColor;
                                pdfCell.textColor = '#FFFFFF';
                                if (pdfCell.font) {
                                    pdfCell.font.name = 'TwCenMT';
                                    pdfCell.font.style = 'bold';
                                    pdfCell.font.size = customFontSize + 2;
                                }
                            }
                            if (gridCell.rowType === 'group') {
                                ...
                                pdfCell.textColor = headerBgColor;
                            }
                    ...
                    }).catch(function (gridError) {
                        newTab.close();
                        empr_helper.notify('Grid export failed: ' + gridError.message, 2);
                    });
                };

            }).catch(err => {
                newTab.close();
                empr_helper.notify('Failed to load assets (Fonts/Logo) from server.', 2);
                console.error(err);
            });

        } catch (globalError) {
            if (newTab) newTab.close();
            empr_helper.notify('An unexpected error occurred.', 2);
        }
    },
    ...
    //    const companyName = empr_helper.companyName;
    //    //const logoUrl = '/Client/Company/logoOri.png';
    //    const reportName = isSubReport ? 'Mpo Layout Summary' : CurrentReportName;
    //    const rawFromDate = $("#FROM_DATE").val();
    //    const fromDate = rawFromDate.split('-').reverse().join('-');
    ...
    //                // Reset state before autotable renders (safety)
    //                doc.setTextColor('#000000');
    //                doc.setFont('TwCenMT', 'normal');
    //                // =====================================================================
    ...
    //            .catch(err => {
    //                newTab.close();
    //                empr_helper.notify('Failed to load Tw Cen MT font from server.', 2);
    //                console.error(err);
    //            });
    ...
    oldPdfMethod: function (isLandscape) {
        debugger;
        $("#Loader").show();
        ...
        const isPartyLedger = empr_helper.reportName === "Master Plaining Sheet";
        if (isPartyLedger) {
            const grid2 = $("#SubGridContainer").dxDataGrid("instance");
            if (grid2) grids.push({ instance: grid2, title: "Mpo Layout Summary" });
        }
        ...
        console.log('requestGrids', requestGrids);
    ...
    DxGridBindingForReportsWithSetting_Aging: function (div, columns, datasrc, fileName, isLandscape = false, isSubReport = false, exportPDF) {
        debugger;
        const columnChooserModes = [{
            "key": 'dragAndDrop',
            "name": 'Drag and drop',
        ...
        const dataGrid = $(div).dxDataGrid({
            ...
            onExporting(e) {
                ...
                if (e.format === 'pdf') {
                    if (exportPDF == 'new') {
                        empr_helper.exportGridToPdf(e.component, false, 6, fileName, isSubReport);
                    }
                    else {
                        empr_helper.oldPdfMethod(isLandscape);
                    }
                } else {
                    // ---------- EXCEL EXPORT ----------
                    const workbook = new ExcelJS.Workbook();
